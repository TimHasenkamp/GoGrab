...
{#if confirmStore.current}
  ...
  >
    <div class="w-full max-w-sm rounded-xl bg-white p-5 shadow-2xl">
      <h2 id="confirm-title" class="text-base font-semibold text-slate-900">{c.title}</h2>
      <p class="mt-1.5 text-sm text-slate-600">{c.body}</p>
      <div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        ...
        <button
          ...
          class="rounded-md px-3 py-1.5 text-sm font-medium text-white shadow-sm {c.destructive
